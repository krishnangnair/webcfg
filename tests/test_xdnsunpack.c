...
#include "../src/xdns_param.h"
#include "../src/comp_helpers.h"

void xdnsUnpack(char *blob);
...
void test_xdns_unpack()
{
	int len=0;
	char subdocfile[64] = "../../tests/xdns.bin";
...
void xdnsUnpack(char *blob)
{
	...
	if(blob != NULL)
	{
		...
		{
			xd = xdnsdoc_convert(decodeMsg, size);//used to process the incoming msgobject
			err = errno;
			printf( "errno: %s\n", xdnsdoc_strerror(err) );
		
			if(xd != NULL)
			{
				printf("xd->subdoc_name is %s\n", xd->subdoc_name);
				printf("xd->version is %lu\n", (long)xd->version);
				printf("xd->transaction_id %lu\n",(long) xd->transaction_id);
				printf("xd->param->enable_xdns %s\n", (1 == xd->enable_xdns)?"true":"false");
				printf("xd->default_ipv4 %s\n",xd->default_ipv4);
				printf("xd->default_ipv6 %s\n",xd->default_ipv6);
				printf("xd->default_tag %s\n",xd->default_tag);
				for(i =0; i< (int) xd->table_param->entries_count; i++)
				{
					printf("xd->table_param->entries[%d].dns_mac %s\n",i, xd->table_param->entries[i].dns_mac);
					printf("xd->table_param->entries[%d].dns_ipv4 %s\n",i, xd->table_param->entries[i].dns_ipv4);
					printf("xd->table_param->entries[%d].dns_ipv6 %s\n",i, xd->table_param->entries[i].dns_ipv6);
					printf("xd->table_param->entries[%d].dns_tag %s\n",i, xd->table_param->entries[i].dns_tag);
				}
			}
			xdnsdoc_destroy(xd);
		}
...
void add_suites( CU_pSuite *suite )
{
    *suite = CU_add_suite( "tests", NULL, NULL );
    CU_add_test( *suite, "test_xdns_unpack", test_xdns_unpack);
}
