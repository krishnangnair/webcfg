...
typedef struct
{
    char wtypestring[256];//portforwarding or wlan
    char val[10];//true or false;
}SubDocSupportMap_t;
...
WEBCFG_STATUS isSubDocSupported(char *subDoc)
{
	...
	{
		if(strncmp(sd[i].wtypestring, subDoc, strlen(subDoc)) == 0)
		{
			WebcfgDebug("The subdoc %s is present\n",sd[i].wtypestring);
			if(strncmp(sd[i].val, "true", strlen("true")) == 0)
			{
